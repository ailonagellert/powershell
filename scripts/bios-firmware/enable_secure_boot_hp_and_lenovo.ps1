...
function Enable-Lenovo-SecureBoot {
    ...
    $result = (Get-WmiObject -Namespace root\wmi -Class Lenovo_SetBiosSetting).SetBiosSetting("WindowsUEFIFirmwareUpdate,Enable")
    }
 ...
 function Reorder-BootOrder {
    ...
    $remainingDevices2 = foreach ($device in $remainingdevices) { $device.Trim() + ":" }

    # Combine prioritized and remaining devices
    $reorderedDevices = $priorityDevices + ":" + $remainingDevices2
    $newBootOrder = "BootOrder," + ($reorderedDevices)

    # Apply the new boot order
    $result = (Get-WmiObject -Class Lenovo_SetBiosSetting -Namespace root\wmi).SetBiosSetting("BootOrder,NVMe0:USBHDD: USBCD: USBFDD: PXEBOOT: LENOVOCLOUD: ON-PREMISE").Return
